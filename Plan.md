...
(auth + F&F follows, §6) is the next milestone. The optional Slack alert
webhook ($5/$10/$20 thresholds) was operator-wired 2026-06-16 — the last
cross-cutting nice-to-have is now done.
...
- [x] **Hard rule enforcement** — landed inside the Phase C Week 4 Risk Gateway (#94 + #105). Per-persona caps live in `risk/gateway.gate()` (single-name, sector, parametric VaR99 vs calibrated caps, drawdown floor); reject-reasons feed the construction retry. Schema-level global `target_weight ≤ 0.20` cap also in place. See §5 Week 4 entry "Subsumes Phase B's deferred Hard rule enforcement task — single home, single code path."
...
- [x] **Cost cap** — fully shipped. Functional component 2026-06-02 (`check_daily_budget()` hard-pauses on cap breach; cost logged per call to `llm_call_log`). Grafana dashboard live since 2026-06-12 (`docs/grafana/llm-cost-dashboard.json`, 7 panels over `llm_call_log`, see CLAUDE.md §2). Slack alert webhook at $5/$10/$20 thresholds operator-wired 2026-06-16 (the hard-pause still gates real spend; the alert is the earlier warning).
...
- [x] **Cost observability — Grafana dashboard LIVE 2026-06-12** (operator setup per `docs/runbooks/observability-grafana-voyage.md`: read-only `grafana_ro` role + datasource + `docs/grafana/llm-cost-dashboard.json` import — 7 panels over `llm_call_log`). Alert rules at $5/$10/$20 (Slack contact point) operator-wired 2026-06-16. Original scope: (rolled forward from Phase B Week 2 plan). Source data already populated in `llm_call_log`; dashboard + webhook wiring both done. Alert thresholds: $5/day (info), $10/day (warning), $20/day (page). The hard pause (`check_daily_budget`) stays as the safety net — alerts give earlier warning.
...
- [x] **Cross-source disagreement dashboards** — shipped 2026-06-14. New `cross_source_disagreements` table (migration `008_*.sql`); `cross_validated()` accepts an optional `event_sink: list[DisagreementEvent]` and appends one record per disagreement (feature/ticker/spread/decision/candidates). `build()` collects per-run and batch-persists via `_persist_disagreements()`. `docs/grafana/cross-source-disagreements-dashboard.json` defines 6 panels over the new table: 24h count / distinct tickers / max spread / top-25 disagreement-prone (ticker, feature, avg + max spread, last_seen) / per-day timeseries by feature / last 50 events with candidates JSON. Same Neon read-only datasource as `llm-cost-dashboard.json`; operator imports following the existing runbook. Auto-extends as `cross_validated()` gets reused for debt_to_equity / gross_margin / FCF — no panel changes needed, just `event_sink=` at the new call sites.
...
- [x] **Leaderboard tab** reads from `persona_performance` — **shipped 2026-06-12**: real 1y† (hypothetical-blended, footnoted) / 90d / sharpe_30d / mdd_30d / NAV per persona via `/api/performance`. **hit_rate live as of 2026-06-14** via FIFO closed-lot tracking in `paper_engine.closed_lots_hit_rate` (pure function, 8 unit tests covering FIFO ordering, multi-lot sells, per-ticker independence, and the no-sells-yet `None` case so the UI renders '—' until real closes exist). `lib/mock/performance.ts` DELETED.
...
- [x] **Backtest mode**: replay 90 days → simulate 90 days of paper trades → baseline Sharpe/MDD — **shipped 2026-06-14**. `jobs/backtest_baseline.py` (#132 with the harness + cost_namespace isolation) replays weekly v2 books over 13 weeks × 4 personas via `run_research` + `construct_portfolio` (persist=False), simulates hold-and-rebalance on `paper_engine.compute_rebalance`/`mark_to_market`, derives per-persona Sharpe + MDD off the daily equity curve. Live run 2026-06-14 (9/13 completed before prod daily-cap collision, since fixed by cost_namespace): Warren 1.28 / Cathie 3.21 / Peter 2.81 / Ray 1.96 — see §5 Acceptance for the full table + framing.
...
- [→] **Push notification on rebalance**: FCM → browser — **deferred to Phase D** (§6). Blocked on user auth + FCM token storage; without followers there is nobody to notify. Single line in Phase D §6: "FCM push when followed persona rebalances".
...
- [x] **fcf_yield precision edge cases** (Quant) — resolved 2026-06-14 via PRs #70 + #121 + #128. Verified live against prod Neon (06-14):
    - **AAPL 3.02%** — baseline control, in-band ✅
    - **NVDA 2.40%** — fixed via #70 (EDGAR concept-priority drop-the-`break`) ✅
    - **AMZN -0.61%** — fixed via #121 (fy_end_month anchor); negative value correctly reflects 2025-2026 AI capex spike vs operating cash flow ✅
    - **COIN None** — fixed via #128 (FCF staleness guard). Diagnostic found EDGAR's standard `freeCashFlow` concept stopped resolving for COIN since 2023-09 (XBRL tag change at issuer); loader was walking back to a 2021 bull-run row ($4.16B) and shipping that as 11.71% yield. New `sum_ttm_fcf(as_of=...)` returns None when newest non-null FCF row > `FCF_STALENESS_MAX_DAYS=400` ≈ 13 months. NULL fcf_yield is honest; LLM prompt now omits the field for COIN until issuer fundamentals freshen up. See `docs/case-studies.md` **CS-13** for full diagnosis + lesson.
    - **UNH 5.30%** — **diagnosis confirmed: no bug. Our GAAP TTM math is correct.** $19.67B trailing FCF / $371B mcap = 5.30% reflects the actual reported series (2022 $24B / 2023 $25B / 2024 $20.7B / 2025 $16.07B — a 5y average ~$21B / mcap $371B is ~5.6%, so 5.30% is the normal converged value). The "consensus ~3%" cited in earlier Plan revisions appears to have been a forward/normalized estimate (strips 2024 cyber-attack recovery and projects future steady-state) — comparing it to our trailing GAAP yield was apples-to-oranges. This item was an unverified suspicion in the original Phase B note; today's diagnostic falsified it. **Nothing to fix.**
  - The remaining ambition (dedicated daily mcap source via FMP `key_metrics` for further precision) is met — `ingestors/fmp_key_metrics.py` ships and adds the 5th mcap candidate (Plan §5 Week 4 item earlier in this file). **Optional Phase D enhancement** (not a fix): publish a parallel normalized-FCF series for healthcare / consumer-defensive names that strips one-time items — would let the LLM cite either trailing GAAP or normalized yield depending on the persona's mandate.
...
| Cathie sector cap | **0.70 → removed (none)** | 2026-06-15 | The sector cap was the wrong tool for this persona — tech/S-curve concentration IS her mandate, not a risk to fence. `max_sector >= 1.0` now means "no cap"; the gateway skips the sector check for her. Risk stays hard-bounded by single-name 16% + VaR99 ≤ 8.5% + the 35% drawdown floor. (CS-11 lesson intact: drop a cap because it's the wrong tool, not because the LLM keeps busting it.) |
| Cathie max positions | **20 → 12** | 2026-06-15 | Focused, highest-conviction book rather than a thin 20-name spray. Enforced as a HARD count ceiling in construction (prompt-only guidance gets ignored under role immersion); candidate shortlist stays 14 |
| Follow model | **Single-follow (one analyst at a time)** | 2026-06-16 | A follower mirrors one persona's $100K paper book; following another switches (drops the prior follow, logged in `follow_events` so the account curve shows the switch). Simpler pilot UX than N independent $100K books; avoids "$300K portfolio" confusion. `/api/follow` POST enforces it server-side |
