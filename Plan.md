...
- [x] **Historical EPS / margin trend ingestor** — shipped 2026-06-09 as a separate `ingestors/yf_history.py`. Calls `yf.Ticker(t).income_stmt`, extracts diluted EPS + revenue + grossProfit + operatingIncome per fiscal year, upserts one synthetic income row per fy_end tagged `source='yfinance_history', period='FY'`. JSONB-merge order keeps EDGAR / FMP canonical values on overlap; yf only fills blanks. Wired into `ingest_daily.py` as the `yf_history` step with a Friday-only cadence guard (`date.today().weekday() == 4`) because yfinance's income_stmt endpoint is slow + rate-limited. `compute.py::_annual_income_rows` already filters on `period='FY'` so the existing `compute_eps_cagr_3y` + `compute_gross_margin_trend` paths consume the rows with zero compute change. Acceptance: V should now show non-None values for both metrics after the next Friday cron (or a manual `yf_history.ingest(['V'])` + `build(['V'])` for immediate verification).
