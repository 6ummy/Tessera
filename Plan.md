...
- [ ] **Historical EPS / margin trend ingestor** — `eps_cagr_3y` and `gross_margin_trend` still N/A for filers whose XBRL doesn't tag `EarningsPerShareDiluted` / `GrossProfit`. yf.Ticker(t).income_stmt returns the last 4 annual periods of GAAP-style line items including diluted EPS — enough to compute a 3-year CAGR for the gap-cases. Plan: extend `yf_shares.py` (rename `yf_fundamentals.py`?) to call `Ticker.income_stmt` quarterly, materialize the rows as `source='yfinance', period_end=fy_end` synthetic income rows so `_annual_income_rows` picks them up via the existing fall-through. Rate-limit concern (yf 4 rps soft cap) → run this on a weekly cron, not the daily orchestrator.
