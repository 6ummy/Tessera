...
- [ ] **Flip `FEATURE_PAPER_EXECUTION=true`** — operator action: after one verified run, change the env in `deploy_cloud_run.ps1` + redeploy. Until then the `paper` step logs `skipped_reason`.
