...
- [x] **fcf_yield precision edge cases** (Quant) — resolved 2026-06-14 via PRs #70 + #121 + #128. Verified live against prod Neon (06-14):
    ...
    - **UNH 5.30%** — intentionally left as-is. Diagnostic confirmed math is correct: $19.67B GAAP TTM FCF / $371B mcap (0.91B shares × $408 close). The "consensus ~3%" cited in earlier Plan revisions is forward/normalized (strips 2024 cyber-attack recovery) — a different metric than trailing GAAP. Sanity bound (±100%) passes; closing the GAAP-vs-normalized gap requires defining a normalized-FCF series, which is a Phase D-class workstream.
  - The remaining ambition (dedicated daily mcap source via FMP `key_metrics` for further precision) is met — `ingestors/fmp_key_metrics.py` ships and adds the 5th mcap candidate (Plan §5 Week 4 item earlier in this file). Carry-over to Phase D backlog: normalized-FCF series for healthcare/consumer-defensive names where management guidance excludes one-time items.
