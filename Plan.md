...
- [x] **Cost cap** — fully shipped. Functional component 2026-06-02 (`check_daily_budget()` hard-pauses on cap breach; cost logged per call to `llm_call_log`). Grafana dashboard live since 2026-06-12 (`docs/grafana/llm-cost-dashboard.json`, 7 panels over `llm_call_log`, see CLAUDE.md §2). Slack alert webhook at $5/$10/$20 thresholds remains optional Phase D nice-to-have (the hard-pause already gates real spend; the alert is just earlier warning) — operator can wire when convenient.
