...
# Notification email sender. AFTER Resend domain verification, change this to
...
& gcloud run deploy $SERVICE `
    ...
    --timeout 3600 `
    --no-cpu-throttling `
    --set-env-vars "ENV=production,LOG_LEVEL=INFO,SENTRY_ENVIRONMENT=production,FEATURE_REAL_LLM=true,FEATURE_PAPER_EXECUTION=true,FEATURE_LIVE_TRADING=false,FEATURE_ALPACA_PAPER_EXECUTION=false,FEATURE_FCM_PUSH=false,FEATURE_EMAIL_NOTIFY=true,GCS_BUCKET_RAW=tessera-raw,LLM_MAX_DAILY_COST_USD=5.0,LLM_MAX_DAILY_COST_CHAT_USD=2.0,ALPACA_BASE_URL=https://paper-api.alpaca.markets,RELY_ON_IAM=true,EMAIL_FROM=$EMAIL_FROM" `
    --set-secrets "DATABASE_URL=DATABASE_URL:latest,ANTHROPIC_API_KEY=ANTHROPIC_API_KEY:latest,ALPACA_API_KEY=ALPACA_API_KEY:latest,ALPACA_API_SECRET=ALPACA_API_SECRET:latest,FMP_API_KEY=FMP_API_KEY:latest,FRED_API_KEY=FRED_API_KEY:latest,NEWSAPI_API_KEY=NEWSAPI_API_KEY:latest,SENTRY_DSN=SENTRY_DSN:latest,WORKER_WEBHOOK_SECRET=WORKER_WEBHOOK_SECRET:latest,SEC_USER_AGENT=SEC_USER_AGENT:latest,VOYAGE_API_KEY=VOYAGE_API_KEY:latest,RESEND_API_KEY=RESEND_API_KEY:latest,UNSUBSCRIBE_SECRET=UNSUBSCRIBE_SECRET:latest"
