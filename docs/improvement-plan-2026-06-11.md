...
5. ✅ Grafana + Voyage — **완전 완료 2026-06-12**: repo 측(`docs/grafana/llm-cost-dashboard.json` 7패널 + `docs/runbooks/observability-grafana-voyage.md`)에 이어 **운영자 콘솔 셋업도 완료** — Grafana Cloud datasource + 대시보드 import, `VOYAGE_API_KEY` secret 생성 + deploy 스크립트 `--set-secrets` 등재 + 재배포. 이제 prod chat 메모리 recall이 recency가 아닌 Voyage 유사도(`sim=` 태그)로 동작. cross_validated 불일치 패널은 로그 기반 메트릭 필요 — 별도 추적.
