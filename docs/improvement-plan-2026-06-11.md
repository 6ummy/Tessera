...
2. ✅ 리스크 게이트웨이(`risk/gateway.py`) — **shipped 2026-06-11**. 얇은 검증기: 유니버스 멤버십(반-환각 최종 관문), sum=1.0·single-name cap 재확인, 그리고 그동안 프롬프트로만 존재하던 **sector cap 강제**. `construct_portfolio` retry 루프에 연결되어 거부 사유가 LLM 재시도 피드백으로 전달됨. cash 범위·conviction floor는 soft(로그만). **후속 완료 2026-06-12 (#105)**: VaR99(실측 보정 캡) + drawdown floor + Ray `gate_regime` — 게이트웨이 풀스택. 부속: weight 텔레메트리(#106), attribution(#107), Sentry 페이징(#108).
...
4. ✅ 프론트 mock 교체 — **shipped 2026-06-12**. `lib/mock/performance.ts` 삭제; 워커 `GET /api/performance/{persona}`(정규화 곡선 + 포인트별 hypothetical 플래그 + 지표) / `GET /api/portfolio/{persona}`(실제 스냅샷만) + Edge 프록시 2개 + `lib/performance-data.ts`(모듈 캐시 훅). 랜딩 히어로·디테일 시트·대시보드(리더보드+포트폴리오 탭) 전부 실데이터. `CumulativeChart`를 date-merge로 수정(실데이터 캘린더 불일치에서 index-merge가 조용히 어긋남). **표기 정책 변경(제품 결정 2026-06-12, 같은 날 후속)**: 최초 출하는 hypothetical 구간 점선+캡션이었으나, 페르소나당 **단일 실선**으로 통합하고 캡션은 "real fills since Jun 11, 2026"만 표기. `hypothetical` 플래그는 DB/API에 그대로 유지. 벤치마크는 기존 `/api/prices/SPY` 재사용. sharpe/mdd는 라이브 트랙 성숙(5일) 전까지 hypothetical 트레일링 값으로 서빙.
   - 4-a. ✅ **고정 book 1년 백필 — shipped 2026-06-12** (결정: frozen-book, look-ahead bias 감수+라벨링 / replay 백테스트는 비용 문제로 기각). `007_hypothetical_track.sql`(두 테이블에 `hypothetical` 플래그) + `jobs/backfill_paper_history.py`: 첫 실제 스냅샷의 qty·cash를 그대로 과거 1년에 투영(이음새 연속성이 구조적으로 보장), SPY 거래일 캘린더 + 종가 forward-fill, 성과는 구간 첫날 기준, trades는 절대 생성 안 함, upsert가 `WHERE hypothetical` 가드라 실제 행 덮어쓰기 불가능. **운영: 007 적용 → `--dry-run` → 본 실행 — 완료.** (표기: 플래그는 데이터/API 레벨 유지; UI는 단일 실선 + "real fills since Jun 11, 2026" 캡션 — 위 4번 항목의 제품 결정 참조.)
5. ✅ Grafana + Voyage — **완전 완료 2026-06-12**: repo 측(`docs/grafana/llm-cost-dashboard.json` 7패널 + `docs/runbooks/observability-grafana-voyage.md`)에 이어 **운영자 콘솔 셋업도 완료** — Grafana Cloud datasource + 대시보드 import + **$5/$10/$20 알림 룰 설정 완료**, `VOYAGE_API_KEY` secret 생성 + deploy 스크립트 `--set-secrets` 등재 + 재배포. recall은 주간 배치에서 Voyage 유사도(`sim=` 태그)로 동작. cross_validated 불일치 패널은 로그 기반 메트릭 필요 — 별도 추적.
   - 5-a. ✅ **Sentry 페이지 알림 (paper engine)** — 코드 측 shipped 2026-06-12: per-persona 격리가 예외를 삼켜 Sentry에 한 번도 도달하지 않던 것을 명시적 `capture_exception`(ledger 실패)/`capture_message`(보유 종목 가격 불가 → MTM skip)로 수정. 운영자: runbook §1-2b의 issue alert 룰 1개 생성(5분 간격 페이지).
