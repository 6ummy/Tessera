...
4. ✅ 프론트 mock 교체 — **shipped 2026-06-12**. `lib/mock/performance.ts` 삭제; 워커 `GET /api/performance/{persona}`(정규화 곡선 + 포인트별 hypothetical 플래그 + 지표) / `GET /api/portfolio/{persona}`(실제 스냅샷만) + Edge 프록시 2개 + `lib/performance-data.ts`(모듈 캐시 훅). 랜딩 히어로·디테일 시트·대시보드(리더보드+포트폴리오 탭) 전부 실데이터. **hypothetical 구간은 점선 + 명시 캡션**("current book projected backwards — look-ahead bias"), 라이브 트랙은 실선. `CumulativeChart`를 date-merge로 수정(실데이터 캘린더 불일치에서 index-merge가 조용히 어긋남). 벤치마크는 기존 `/api/prices/SPY` 재사용. sharpe/mdd는 라이브 트랙 성숙(5일) 전까지 hypothetical 트레일링 값으로 서빙.
   - 4-a. ✅ **고정 book 1년 백필 — shipped 2026-06-12** (결정: frozen-book, look-ahead bias 감수+라벨링 / replay 백테스트는 비용 문제로 기각). `007_hypothetical_track.sql`(두 테이블에 `hypothetical` 플래그) + `jobs/backfill_paper_history.py`: 첫 실제 스냅샷의 qty·cash를 그대로 과거 1년에 투영(이음새 연속성이 구조적으로 보장), SPY 거래일 캘린더 + 종가 forward-fill, 성과는 구간 첫날 기준, trades는 절대 생성 안 함, upsert가 `WHERE hypothetical` 가드라 실제 행 덮어쓰기 불가능. **운영: 007 적용 → `--dry-run` → 본 실행.** UI는 hypothetical 구간에 "Hypothetical — current book held 1y" 라벨 필수.
