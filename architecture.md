...
- **RESOLVED 2026-06-13** — option (b) shipped. `ingest_daily` and `persona_batch` now run as **Cloud Run Jobs** (run to completion regardless of HTTP lifecycle), triggered by Cloud Scheduler. The Service keeps the HTTP surface (`/api/*`, chat) + the `/jobs/*` endpoints as a manual fallback. See `apps/worker/scripts/deploy_cloud_run_jobs.ps1` + `docs/runbooks/cloud-run-jobs.md`.
