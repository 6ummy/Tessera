...
| **All / PM / Persona Voice** | 정우 | 전체 코디네이션, 페르소나 voice 게이트키퍼, compliance 준비, 모든 PR 참여 |
...
- `personalities.md` → **정우만** 변경 권한 (CODEOWNERS로 강제). 페르소나 voice는 디자인 결정, 분산되면 일관성 깨짐.
...
| **정우 (PM/Voice)** | Persona voice eval set (10 thesis × 4 페르소나 = 40개) + 변호사 outreach 시작 | `tests/persona_voice/eval_set.yaml` + lawyer 1차 상담 일정 |

**Week 1 종료 조건**: Anthropic 호출 1개 성공, Cloud Run 배포 완료, mobile 깨짐 0건.
...
# 4. 환경변수 받기
cp .env.example .env
# → 정우(@6ummy)에게 .env 값 요청 (Notion or 1Password로 공유 예정)
...
> Anthropic 키는 dev/staging/prod 분리합니다. 본인용 dev 키 받아서 `.env`에. Production 키는 Cloud Run Secret Manager에만 존재.

---

## 6. 커밋 메시지 컨벤션 (가볍게)

100% 강제 안 합니다. 다만 머지 메시지(squash 제목)는 트랙 prefix 권장:
...
1. **Production API key를 dev/local에 절대 쓰지 마세요.** 본인용 dev 키 발급받아서 사용.
...
4. **Anthropic 호출 추가 PR은 cost 추정 댓글 필수.** 예: "이 endpoint는 메시지당 ~$0.012, hourly chat 사용 시 ~$5/day 증가."

위 셋 중 하나라도 어기면 머지 거부 (정우 권한).
...
이 문서는 살아있는 운영 메뉴얼입니다. 이상하거나 어색한 부분 발견하면
